...
class AddMissionViewModel(ViewModelBase):
    def __init__(self):
        ...
        self.info = self.request_dict.info

    def validate(self):
        if not self.missionid or not self.name.strip() or not self.start or not self.end or not self.info:
            self.error = 'Please fill in all fields'
        missions = mission_service.mission_ids()
        id_list = [y for x in missions for y in x]
        self.missionid = int(self.missionid)
        if self.missionid in id_list:
            self.error = 'Mission ' + str(self.missionid) + ' already exist'
